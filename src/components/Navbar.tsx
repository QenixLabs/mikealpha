...
import { Menu, X, ChevronDown, Leaf, CloudRain, Shield, Microscope, Droplets, Sprout, FlaskConical } from "lucide-react"

const productChildren = [
  { label: "Foliar Solutions", path: "/products?category=Foliar+Solutions", icon: <Droplets className="w-5 h-5" />, desc: "Direct leaf nutrition" },
  { label: "NPK Fertilizers", path: "/products?category=NPK+Fertilizers", icon: <Sprout className="w-5 h-5" />, desc: "Complete plant food" },
  { label: "Biological Fertilizers", path: "/products?category=Biological+Fertilizers", icon: <Leaf className="w-5 h-5" />, desc: "Bio-enhanced growth" },
  { label: "Straight Fertilizers", path: "/products?category=Straight+Fertilizers", icon: <FlaskConical className="w-5 h-5" />, desc: "Single nutrient focus" },
  { label: "Micronutrients", path: "/products?category=Micronutrients", icon: <Microscope className="w-5 h-5" />, desc: "Trace element boost" },
]
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (!hero) return
      setHeroVisible(hero.getBoundingClientRect().bottom > 72)
    }
  ...
  const transparent = isHome && heroVisible && !mobileOpen
    ...
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-out ${
          transparent
            ? "bg-transparent"
            : "bg-white/85 backdrop-blur-xl border-b border-[#E5E5E0]/60 shadow-[0_1px_3px_rgba(0,0,0,0.04)]"
        }`}
        style={{ height: heroVisible ? 72 : 64 }}
      >
              ...
              {navLinks.map((link) =>
                link.children ? (
                  ...
                  >
                    <Link
                      ...
                      className={`flex items-center gap-1 px-4 py-2 text-[13px] font-semibold rounded-lg transition-colors ${
                        pathname.startsWith(link.path)
                          ? transparent ? "text-white" : "text-coral"
                          : transparent
                    ...
                    {/* Mega dropdown */}
                    {productsOpen && (
                      <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 bg-white/95 backdrop-blur-xl rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.1)] border border-[#E5E5E0]/80 p-3 w-[420px] overflow-hidden">
                        <div className="grid grid-cols-2 gap-1">
                          {link.children.map((child) => (
                            <Link
                              key={child.path}
                              href={child.path}
                              className="flex items-start gap-3 px-3 py-3 rounded-xl text-sm text-[#1A1A1A]/70 hover:text-coral hover:bg-coral-subtle/40 transition-all group"
                            >
                              <span className="w-9 h-9 rounded-lg bg-[#F5F5F0] group-hover:bg-coral-subtle flex items-center justify-center shrink-0 text-[#1A1A1A]/50 group-hover:text-coral transition-colors">
                                {child.icon}
                              </span>
                              <div>
                        ...
                        <div className="mt-2 pt-2 border-t border-[#E5E5E0]/60 px-3">
                          <Link
                            href="/products"
                          ...
                          >
                            ...
                            <span className="text-[10px]">46+ formulations</span>
                          </Link>
                        </div>
                      </div>
                    )}
